...
model {
  ...
  if(weight_type == 1){
    for(k in 1:K){
      target += w1t_lpdf(t[k] | 0, beta, df[k], crit_t[k], {t[k], df[k], crit_t[k]}, x_i, tolerance);
    }
  }else if(weight_type == 3){
    for(k in 1:K){
      target += w3t_lpdf(t[k] | 0, beta, df[k], crit_t[k], {t[k], df[k], 2}, x_i, tolerance);
    }
  }else if(weight_type == 4){
    for(k in 1:K){
      target += w3t_lpdf(t[k] | 0, beta, df[k], crit_t[k], {t[k], df[k], 1}, x_i, tolerance);
    }
  }
}
